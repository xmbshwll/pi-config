...
const REVIEW_RUBRIC_SKILL_PATH = path.resolve(
  EXTENSION_DIR,
  "../skills/review-rubric/SKILL.md",
);
...
async function loadReviewRubric(): Promise<string> {
  ...
  try {
    ...
    console.error(
      `[review] Failed to load rubric from ${REVIEW_RUBRIC_SKILL_PATH}: ${err}`,
    );
    cachedReviewRubric =
      "Review the code changes and provide prioritized findings.";
  }

...
function updateBranchStatus(ctx: ExtensionContext) {
  ...
  const pctStr =
    info.contextPercent !== null ? ` (${info.contextPercent.toFixed(1)}%)` : "";
  const label =
    info.count === 1
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerMessageRenderer(
    ...
    (message, { expanded }, theme) => {
      const details = message.details as
        | { count: number; tokens: number; percent: number | null }
        | undefined;
      const count = details?.count ?? "?";
      const tokenStr = details?.tokens ? formatTokens(details.tokens) : "?";
      const pctStr =
        details?.percent !== null && details?.percent !== undefined
          ? ` · ${details.percent.toFixed(1)}% context`
          : "";
      ...
      const label = theme.fg(
        "customMessageLabel",
        `\x1b[1m[${count}x branches · ~${tokenStr} tokens${pctStr}]\x1b[22m`,
      );
      ...
      if (expanded) {
        ...
        box.addChild(
          new Text(theme.fg("dim", "  expand to see full summary"), 0, 0),
        );
      }
  ...
  pi.registerCommand("branch", {
    ...
    handler: async (_args, ctx) => {
      ...
      if (leafId === branchFromId) {
        ctx.ui.notify(
          "Already at a branch point — nothing to branch from",
          "info",
        );
        return;
      }
  ...
  pi.registerCommand("compress", {
    description:
      "Summarize the entire session and start a new session with the summary (original session is preserved)",
    handler: async (_args, ctx) => {
      await ctx.waitForIdle();
      ...
      const trimmedConversation =
        conversationText.length > maxConversationChars
      ...
      try {
        ...
        if (response.stopReason === "error") {
          ctx.ui.notify(
            `Summary generation failed: ${response.errorMessage || "unknown error"}`,
            "error",
          );
          return;
        }
        ...
        if (result.cancelled) {
          ...
          updateBranchStatus(ctx);
          ctx.ui.notify(
            "Started fresh session with conversation summary",
            "success",
          );
        }
      } catch (err) {
    ...
    handler: async (_args, ctx) => {
      ...
      const pctStr =
        info.contextPercent !== null
          ? ` (${info.contextPercent.toFixed(1)}% context)`
          : "";
